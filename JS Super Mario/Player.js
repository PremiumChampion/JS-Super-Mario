...
class player
{
    constructor(posx, posy, playerDirection_)
    {
        ...
        this.positionx = posx;
        this.positiony = posy;
        this.playerDirection = playerDirection_;
        this.falling = false;
        this.itm;
        this.live = 1;
        }
    ...
    GetPosition(margin)
    {
        ...
        var re = Math.floor((poscentery + 0)/16) * 50 + Math.floor((poscenterx + 0) / 16);
    ...
    SetBuff(item){
        ...
        if(item != "LIVE")
        {
            ...
            var img = new Image();
            switch (item) {
                case "FIRE": img.src = 'Img/FR.png'; itmTimeoutTime *= 10; break;
                case "STAR": img.src = 'Img/STR.png'; itmTimeoutTime *= 4; break;
                case "SPEED": img.src = 'Img/SPD.png'; itmTimeoutTime *= 4; break;
                case "FREEZE": img.src = 'Img/FRZZ.png'; itmTimeoutTime *= 4; break;
             }
            mario.itm = item;
            clearTimeout(itemTimeout);
            ctx.clearRect(0,0,16,16);
            ctx.drawImage(img,0,0);
            itemTimeout = setTimeout(function(){mario.itm = "";
            ctx.clearRect(0,0,16,16);
            }, itmTimeoutTime);
        }
        else
        ...
        if(mario.itm === "ENEMY")
        {
            enemys.push(new enemy(this.GetPosition(8)-50));
        }
    }
